...
def LeastSquare(trainList):
	...
	weightList, lse, alpha, n =[], [2, 1, 1], 0.001, len(trainList)
	...
	for j in range(nofeatures):
		...
		weightList.append(random.randrange(0,1))
...
#			alpha = alpha / 10
		...
		for i in range(nofeatures):
			...
			weightList[i] += alpha * (1 / (n-1)) * loss(outPut, trainList, weightList)[0][i]
			
		lse = [loss(outPut, trainList, weightList)[1]] + lse
		#previous error - current error
		print(lse[2]-lse[0],lse[0])
